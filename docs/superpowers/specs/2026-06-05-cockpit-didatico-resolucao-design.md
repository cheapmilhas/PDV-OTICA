...
export interface Issue {
  id: string;                  // estável p/ key + dedupe (ex.: "billing_sync:<companyId>")
...
| 5 | **Trial vencendo/vencido** | `status==="TRIAL"` e `trialEndsAt` ≤ +3 dias (ou já passou) | vencido→warning, vencendo→info | `kind:"blueprint"` → `extend_trial` |
...
Função `getProblemCompanies()` que faz **uma query Prisma** buscando só empresas em algum estado problemático (filtro amplo: blocked OU health crítica OU trial OU pastDue OU billingSyncPending OU com fatura overdue), com `take` de segurança. Mapeia para `ProblemCompany[]`. Multi-tenant N/A (visão super-admin, cross-tenant intencional, como o resto do cockpit).
...
- **`issue-card.tsx`** (NOVO): renderiza um `Issue`. Botão "Resolver":
  - `kind:"blueprint"` → abre o `ActionModal` (carrega o descritor do blueprint via `/api/admin/actions`, injeta `companyId`/`companyName`).
  - `kind:"link"` → navega para o `href`.
  - `kind:"info"` → executa a ação leve (ex.: refetch do pulse) ou só explica.
