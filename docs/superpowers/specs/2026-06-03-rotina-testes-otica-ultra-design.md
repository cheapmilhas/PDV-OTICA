...
- **Correção:** trocar a string pelo nome do usuário logado (sessão / `useSession`).
...
2. PATCH: receber e validar `financeAccountId`.
...
### Onde aplicar
1. **Laboratórios** — `laboratories/route.ts:101-114` (`_count.serviceOrders`) e `laboratories/[id]/service-orders/route.ts:28-51`. Backend decide pelo papel: vendedor → `status: { not: "CANCELED" }` no count e na lista; admin/gerente → tudo, marcando canceladas.
2. **Contas a Receber** — `accounts-receivable/route.ts:88-196` (hoje, sem param `status`, retorna todos). Vendedor → excluir CANCELED por padrão; admin/gerente → incluir com badge.
...
- **Correção (migration aditiva):**
  1. Migration: `Sale.number Int` + `@@unique([companyId, number])` (espelha `ServiceOrder` `schema.prisma:855/917`).
  2. Backfill: numerar vendas existentes por `createdAt` por empresa; semear counter `"sale"` no máximo atual.
  3. `sale.service.ts` create: chamar `getNextSequence(companyId, "sale", tx)` dentro da transação, gravar em `Sale.number`.
  4. Criar `src/lib/sale-number.ts` (`saleDisplayNumber(sale)` → `#000123`, fallback CUID para legado onde `number === 0`).
  5. Trocar todos os `sale.id.substring/slice` pelo helper (listas, recibos, carnê, e descrição "Parcela 2/4 - Venda #..." em `sale-side-effects.service.ts:253,276`).
  6. Remover o `as any` em `pdf/route.ts:104`.
...
- **Atenção:** migration + backfill é a parte mais sensível do sprint. Testar com cuidado redobrado; é aditiva, não destrutiva.
