...
**Bug de cor real:** `.bg-brand-primary/10` e similares em `globals.css` apontam para roxo/ciano legados (#6366F1/#0EA5E9). `security.tsx` usa essas classes → ícones fora da marca. Corrigir tokens para azul/ciano Vis (#2E6BFF/#22C3E6).

**Padronização:**
- `<SectionHeading>` (`src/components/home/section-heading.tsx`): eyebrow (text-xs font-bold uppercase tracking-[0.15em] text-brand-primary) + h2 (font-heading font-extrabold var(--text-h1) leading-[1.1]) + subtítulo (mt-4 max-w-xl text-muted). Wrapper text-center mb-12 md:mb-16. Refatorar seções para usar.
- `.vis-card` em `@layer components`: rounded-2xl p-6, background var(--lp-surface), border var(--lp-border); hover translateY(-4px) + border-hover + box-shadow 0 8px 28px rgba(10,31,68,0.10). Substituir hovers feitos via `onMouseEnter`/`.style` direto. Para accent por cor, usar var --card-accent.
...
- Corrigir `src/content/pricing.ts`: remover features falsas (NF-e/NFC-e, WhatsApp automático); "integração com laboratórios" → "gestão/controle de laboratórios"; alinhar contagem de features (15, cashback é base).
...
- Componentes reutilizáveis: `<BrowserFrame>` (extrair do hero), layout-padrão da feature.
- Hub `/funcionalidades` (em (landing)): hero curto + grid de módulos (cards → Link) + reuso de FinalCta.
- 5 subpáginas: `/funcionalidades/{pdv-para-otica, ordem-de-servico-otica, controle-de-estoque-otica, gestao-financeira-otica, leitura-de-receita-ia}`. Cada uma: hero da feature (split) + tira de benefícios + mockup grande + sub-recursos (bento) + mini-FAQ + FinalCta. Metadata + canonical por página.

**Teste:** tsc + build + screenshot hub + 1 subpágina; navegação hub↔spoke; CTAs corretos.
...
- `sitemap.ts`: +12 páginas novas +/termos +/privacidade.
- JSON-LD: `BreadcrumbList` (subpáginas/artigos/comparativo), `BlogPosting` (artigos), `Product`/`AggregateOffer` em /precos (sem aggregateRating). Breadcrumb visual HTML.
- `public/llms.txt`: +páginas novas + seção do que o Vis NÃO faz.
- Nav: header "Funcionalidades" → /funcionalidades (página real) + "Blog"; footer reescrito com links reais; remover/ajustar /demo morto.
