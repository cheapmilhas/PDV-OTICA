...
- **F3.3 — Grant + sessão sombra + enforcement allow-list (Domus).** RE-FATIADA após o mapa dos internals (toca o auth de TODOS os 116 usuários — sub-fatias isoladas p/ nenhum commit misturar "mexer no auth de todo mundo" com "lógica nova de suporte"):
  - **F3.3a — sidecar `support_sessions`** (session_id→sessions.id, grantId, clinicId, visOperatorRef, absoluteExpiresAt, revokedAt, scope). Migração. SÓ tabela + lib de leitura do grant ativo por session.id (fora do cookieCache). Nenhum branch no auth ainda. Testável isolado.
  - **F3.3b — identidade sombra + criação de sessão.** NÃO há precedente no app (nenhum auth.api.signIn/createSession). Criar user sombra em usersTable (SEM linha em usersToClinicsTable) + mintar sessão via createSession interno + escrever cookie assinado + magic URL 60s single-use ligada ao grant. Consome o grant REDEEMED→SESSION_OPEN.
  - **F3.3c — branch no customSession** (auth.ts:41): quando a sessão é de suporte (sidecar), injeta clinic do GRANT (não da membership) + isSupportSession + scope. 🚨 toca o fold de TODOS → revisão Codex dedicada.
  - **F3.3d — branch no hasPermission/getEffectivePermissions** (check-permission.ts:8/38): DESCOBERTA — releem usersToClinicsTable direto e negam user sem linha; páginas de PHI gateiam por hasPermission, então sem este branch o operador passa nas action clients mas é BLOQUEADO em toda página de PHI. Precisa derivar permissões (SÓ leitura) do grant. 🚨 toca autorização de todos → Codex.
  - **F3.3e — enforcement read-only + kill-switch.** Gate nas action clients de escrita (nega se isSupportSession — v1 read-only) + revogação (admin) que corta na hora via disableCookieCache no getSession. Teste: operador lê PHI, NÃO escreve (403), revogação imediata, expira absoluto 60min.
  - 🔑 FATOS do mapa: data loaders src/data/* NÃO têm guard (confiam no clinicId da página) → a fronteira de tenant é 100% a resolução de clínica injetada; sidecar > colunas na sessionsTable; getSession({query:{disableCookieCache:true}}) força leitura fresca (fecha o furo dos 5min); middleware é Edge só-cookie (não serve p/ escopo).
