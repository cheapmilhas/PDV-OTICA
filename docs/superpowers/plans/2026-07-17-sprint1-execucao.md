...
- [x] **5.1** `POST /api/internal/vis/entitlements`: valida HMAC + janela 5 min; idempotência — se `eventId` já em `vis_entitlement_events`, responde 200 sem reaplicar; descarta se `source_updated_at` <= o gravado (fora de ordem); faz upsert em `clinic_entitlements` (por `clinic_id`), grava o evento como `applied`. **FEITO** (commit Domus `c5fbd4a`; refatorado p/ núcleo compartilhado em `3fbcc74`). O read-then-write foi trocado por upsert condicional atômico (`setWhere`) → à prova de corrida (achado Codex).
...
- [x] **5.2** Cron `GET /api/cron/sync-vis-entitlements` protegido por `Authorization: Bearer CRON_SECRET`. **FEITO** (`3fbcc74`). Correção sobre o plano original (achado Codex): NÃO itera "linhas já em clinic_entitlements" (perderia clínica cujo 1º webhook falhou) → usa a **lista canônica do Vis** (`GET /api/internal/domus/entitlements`, bootstrap) e puxa cada clínica com **cross-check de identidade**. `Promise.allSettled` (falha isolada não aborta batch). Alerta `synced_at > 24h`. `vercel.json` diário 04:20. Env nova `VIS_PULL_URL`.
...
- [x] **5.3** Testes (Domus, vitest): **FEITO** — 24 testes em `tests/vis-entitlements/` (núcleo webhook: replay idempotente, clínica 422, primeiro snapshot, fora-de-ordem; núcleo pull: não grava evento, igual-e-fresco refresca, mais-velho stale, 422; validação de payload; cron: auth Bearer, sem VIS_PULL_URL desabilita, batch com falha isolada, cross-check de identidade, listing 502). Fake DB (sem banco real — DATABASE_URL=prod).
- [x] **5.4** Migração: **nenhuma** — índices do 0040 bastam. O upsert condicional usa a PK existente.
- [x] **5.5** Commit (Domus): `3fbcc74`.

---

## Task 6 — Verificação ponta-a-ponta (sombra) e fechamento

- [x] **6.1** Segredos setados nos 2 painéis (Production). Deploy dos 2 feito. **FEITO 17/07 noite.** 🔥 2 bugs achados no caminho: (a) middleware `proxy.ts` do Vis barrava `/api/internal/*` (fix `a7f7eb4b`); (b) publisher timeout 5s estourava no cold start do Domus (fix `c9fd3696`, 5s→10s).
- [x] **6.2** E2E sombra: **VERDE 17/07 noite.** Ciclo `unblock→write_allowed=true/ACTIVE` e `block→write_allowed=false/SUSPENDED` propagou do Vis pro espelho `clinic_entitlements` do Domus. 2 eventos distintos, ambos applied, SEM reaplicação (idempotência). `synced_at` avança. Detalhe original abaixo mantido para referência.

- [ ] **6.2-ref (referência)** No `/admin` (seletor Vis Medical) mudar a assinatura da Clínica Domus Saúde. **NÃO usar `extend_trial`** — ela exige status TRIAL e o cliente nasce ACTIVE (`actions/route.ts:69`). Usar **bloquear/desbloquear** (a ação `block` seta `isBlocked`, que `checkSubscription` lê em `subscription.ts:78` → `allowed:false`) OU trocar de plano. Ciclo: bloquear → confirmar no Domus que `clinic_entitlements.write_allowed=false`; desbloquear → confirmar `write_allowed=true`. Rodar o cron manualmente e confirmar idempotência: o segundo disparo NÃO duplica evento (`vis_entitlement_events` não cresce) e NÃO muda a DECISÃO (`write_allowed`/`reason` estáveis). **`synced_at` DEVE atualizar** a cada sync bem-sucedido — é o que alimenta o alerta de defasagem > 24 h (Codex: mantê-lo fixo quebraria o alerta). Ou seja: idempotência é sobre a decisão e o evento, não sobre `synced_at`.
