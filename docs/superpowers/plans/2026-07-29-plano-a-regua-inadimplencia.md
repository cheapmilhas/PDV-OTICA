...
4. **"Adiado para sempre" é silencioso.** Se uma assinatura em atraso não tiver fatura-âncora (todas manuais, ou a única foi cancelada/paga no meio do ciclo), nenhuma trilha é escrita → `hasDispatchedNotice` nunca vira verdadeiro → `suspendDeferred` incrementa todo dia sem ninguém saber. Falha na direção segura (protege o cliente) e espelha o `cancelDeferred` que já existia, mas **não há alerta**: os contadores só aparecem no corpo JSON da resposta do cron, que ninguém lê. ✅ Verificado em produção 2026-07-29: **0 faturas sem `dueDate`** e **0 assinaturas em atraso** — o cenário é teórico hoje. Fast-follow sugerido: alertar o operador quando a mesma assinatura adiar N dias seguidos.
