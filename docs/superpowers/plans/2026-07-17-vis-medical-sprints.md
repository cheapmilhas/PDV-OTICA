...
**PRÓXIMA TAREFA → Sprint 1 "Domus como cliente no portal" (a próxima linha da tabela) OU Sprint 2 (o cadeado). SPRINT 1 SYNC = ✅ E2E VERDE 17/07 noite: block/unblock no /admin reflete em `clinic_entitlements` no Domus. ⚠️ Dono: DESBLOQUEAR a Clínica Domus Saúde se ainda estiver bloqueada do teste. // Paralelo dono: bucket R2 (T0.5).**
...
| Sprint 1 — sync Vis→Domus (sombra) | ✅ **E2E VERDE 17/07 noite.** block→`write_allowed=false/SUSPENDED`, unblock→`true/ACTIVE` no espelho `clinic_entitlements`. 2 fixes no caminho: proxy do Vis barrava `/api/internal/*` (`a7f7eb4b`), publisher timeout 5s→10s cold start (`c9fd3696`). |
| Sprint 1 — Domus cliente nº 1 no portal | ⬜ (alternar seletor /admin p/ Vis Medical e ver a clínica como cliente) |
