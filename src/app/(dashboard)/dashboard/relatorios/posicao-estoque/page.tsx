...
function RelatorioPosicaoEstoquePageContent() {
  ...
  const fetchReport = async () => {
    ...
    try {
      ...
      if (belowMinimum) params.set("belowMinimum", "true");

      const res = await fetch(`/api/reports/stock/position?${params}`);
      if (!res.ok) {
  ...
  const handleExportExcel = () => {
    ...
    import("xlsx").then((XLSX) => {
      ...
      XLSX.writeFile(
        wb,
        `relatorio-posicao-estoque-${format(new Date(), "yyyy-MM-dd")}.xlsx`
      );
    });
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold">Posição de Estoque</h1>
              ...
              <div className="flex items-center space-x-2">
                <Checkbox
                  ...
                  onCheckedChange={(checked) =>
                    setBelowMinimum(checked as boolean)
                  }
                />
                <Label htmlFor="belowMinimum" className="cursor-pointer">
      ...
      {data && (
        <>
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            ...
            <KPICard
              title="Produtos OK"
              value={data.summary.productsOK}
              subtitle={`${data.summary.productsLow} baixos, ${data.summary.productsOut} zerados`}
              icon={CheckCircle}
            />
            <KPICard
              title="Alertas"
              value={data.summary.productsLow + data.summary.productsOut}
          ...
          <Card>
            <CardHeader>
              <CardTitle>Produtos ({data.products.length})</CardTitle>
            </CardHeader>
            <CardContent>
                ...
                <Table className="w-full text-sm">
                  <TableHeader>
                    <TableRow className="border-b">
                      ...
                      <TableHead className="text-center p-2">Mínimo</TableHead>
                      <TableHead className="text-right p-2">Custo</TableHead>
                      <TableHead className="text-right p-2">Valor Total</TableHead>
                  ...
                  <TableBody>
                    {data.products.slice(0, 100).map((product) => (
                      ...
                      >
                        ...
                        <TableCell data-label="Mínimo" className="p-2 text-center text-muted-foreground">
                          {product.minimumStock}
                        </TableCell>
                        <TableCell data-label="Custo" className="p-2 text-right">
                          {formatCurrency(product.costPrice)}
                        </TableCell>
                        <TableCell data-label="Valor Total" className="p-2 text-right font-medium">
