...
/**
 * Conteúdo da aba "Ranking" (metas/ranking).
 *
 * `showCommission` é derivado do `mode` recebido pela aba:
 *   - true  (modo legacy) → tela completa de hoje: card "Comissões Totais", aba
 *     "Comissões" (Base/Bônus/Total/Status + "Pagar") e botão "Fechar Mês".
 *   - false (modo new)    → a comissão sai daqui (fica só em Relatórios →
 *     Comissões). Some o card de comissão, a aba e o "Fechar Mês"; restam só o
 *     Ranking e os cards de vendas/metas.
 *
 * A regra nova é GLOBAL (não por-ótica). Esconder a UI é só metade: o backend
 * (POST/PUT /api/goals/commissions) recusa em modo new — ver as rotas.
 */
function GoalsPageContent({ showCommission }: { showCommission: boolean }) {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      // Em modo new a comissão não é exibida aqui — não buscamos o legado.
      ...
      const [dashRes, sellersRes, commissionsRes] = await Promise.all(requests);
      ...
      if (showCommission && commissionsRes) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold flex items-center gap-2">
            ...
            {showCommission ? "Metas e Comissões" : "Metas e Ranking"}
          </h2>
        ...
        <div className="flex items-center gap-2">
          ...
          {/* "Fechar Mês" calcula a comissão legada — só no modo legacy. */}
          {showCommission &&
            hasPermission("goals.manage") &&
            dashboard?.goal &&
            dashboard.goal.status === "ACTIVE" && (
      ...
      >
        ...
        {/* Card de comissão — some no modo new (comissão fica em Relatórios). */}
        {showCommission && (
          <Card>
            <CardHeader className="pb-2">
      ...
      {/* Tabs — no modo new sobra só o Ranking, então sem abas (evita aba única). */}
      {showCommission ? (
        ...
        // Modo new: Ranking direto, sem o invólucro de abas.
        <RankingCard dashboard={dashboard} formatCurrency={formatCurrency} />
      )}
      ...
      {/* Modal Fechar Mês — só montado no modo legacy (botão não aparece em new). */}
      {showCommission && (
...
/**
 * Ranking de Vendedores — vendas/metas, NÃO é comissão. Aparece nos dois modos.
 * Extraído para reusar entre a aba (legacy) e a exibição direta (new).
 */
...
/**
 * Aba "Ranking" da página consolidada de Metas/Comissões/Config.
 *
 * Recebe `mode` (new|legacy) e deriva `showCommission`. O gating de plano
 * (feature "goals") agora é por-aba, então envolvemos em <FeatureGate>.
 * O guard de rota (ProtectedRoute) fica no nível da página.
 */
export function RankingTab({ mode }: { mode: "new" | "legacy" }) {
  ...
  return (
    <FeatureGate feature="goals" featureName="Metas">
      <GoalsPageContent showCommission={showCommission} />
    </FeatureGate>
  );
}
